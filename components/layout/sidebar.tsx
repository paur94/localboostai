...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex w-16 hover:w-64 transition-[width] duration-300 ease-in-out flex-col liquid-glass border-r border-sidebar-border shrink-0 overflow-hidden group/sidebar">
      {/* Logo */}
      <div className="flex h-16 items-center justify-center group-hover/sidebar:justify-start gap-2.5 border-b border-sidebar-border px-4 shrink-0">
        ...
        <span className="font-bold text-sidebar-foreground tracking-tight whitespace-nowrap">{APP_NAME}</span>
      </div>
          ...
          {navItems.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={cn(
                  "group flex items-center justify-center group-hover/sidebar:justify-start gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors duration-200 outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:ring-offset-1",
                  active
              ...
              >
                ...
                {active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary-foreground/60 shrink-0" />}
              </Link>
            );
          })}
        </nav>
      ...
      <div className="overflow-hidden max-h-0 group-hover/sidebar:max-h-44 transition-[max-height] duration-300 ease-in-out">
        <div className="p-4 border-t border-sidebar-border">
          <div className="liquid-glass rounded-2xl p-4 bg-linear-to-br from-primary/15 to-purple-500/10">
            ...
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed whitespace-nowrap">Get 200 generations/month and unlock all features.</p>
            <Link
              href="/pricing"
